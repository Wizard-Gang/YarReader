...
| `npm run audit:dependencies` | Query live high-severity dependency advisories | Requires npm registry access; findings and unavailable queries both fail. CI runs it separately from credential-free `check`. The existing `security:dependency-advisories` command remains an alias. |
